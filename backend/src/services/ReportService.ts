...
export class ReportService {
   ...
   * 获取月度统计数据
   */
  async getMonthlyStats(filter: MonthlyStatsFilter): Promise<MonthlyStatsData[]> {
   ...
   * 获取物品使用排行数据
   */
  async getItemUsageRanking(filter: ItemUsageFilter): Promise<ItemUsageRankingData[]> {
    const client = await pool.connect();
    try {
      ...
      return result.rows.map(row => ({
        ...
        lastUsedDate: formatDate(new Date(row.last_used_date), 'yyyy-MM-dd')
      }));
    } finally {
      client.release();
    }
   ...
   * 获取库存状态报表数据
   */
  async getInventoryStatus(filter: InventoryStatusFilter): Promise<InventoryStatusData[]> {
    const client = await pool.connect();
    try {
      ...
      return result.rows.map(row => ({
        ...
        lastTransactionDate: formatDate(new Date(row.last_transaction_date), 'yyyy-MM-dd')
      }));
    } finally {
      client.release();
    }
   ...
   * 获取交易历史报表数据
   */
  async getTransactionHistory(filter: TransactionHistoryFilter): Promise<{
   ...
   * 获取自定义时间范围的统计摘要
   */
  async getCustomRangeStats(filter: ReportDateRange & {
    locationId?: string;
    category?: string;
  }): Promise<{
    summary: {
